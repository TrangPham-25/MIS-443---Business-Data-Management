...
SELECT SUM(balance) AS total_checking_ FROM accounts 
...
(10 marks)
*/
SELECT c.customer_id , CONCAT(c.first_name,'', c.last_name) AS full_name, SUM(a.balance) AS total_balance
FROM customers c
...
*/
SELECT c.customer_id, CONCAT(first_name,'',last_name) AS full_name, a.account_id, a.account_type, a.balance
FROM customers c
JOIN accounts a ON a.customer_id = c.customer_id
...
SELECT  c.customer_id,CONCAT(c.first_name, ' ', c.last_name) AS full_name, COUNT(t.transaction_id) AS total_transactions,
